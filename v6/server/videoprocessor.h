...
public:
    ...
    void decode()
    {
        try{
            ...
            DECODE_INT_MEM(NearPointDistance);
            DECODE_INT_MEM(FarPointDistance);
            try{DECODE_JSONDATA_ARRAY_MEM(LaneData);
            }catch(exception e){}
            DECODE_JSONDATA_ARRAY_MEM(DetectLine);
            try{
    ...
    bool press(VdPoint pnt)
    {
        for(int i=0;i<LaneData.size();i++){
            if((point_index=p_on_lane(LaneData[i],pnt))>0){
                ...
                prt(info,"mvd press index %d (%x)",point_index,&seizing)
                        return true;
            }
        }
        ...
        for(int i=0;i<Events.size();i++){
            if((point_index=p_on_v(Events[i].Vers,pnt))>0){
                ...
                prt(info,"mvd press index %d (%x %x)",point_index,&seizing,this)
              ...
              B draw_circle,C draw_text)
    {
        ...
        //   prt(info,"szing %d(%x,%x)",seizing,&seizing,this);
        for(EventRegion r:Events){
            ...
            if(r.Vers.size()){
                if(seizing){
                    draw_text("event region",VdPoint(r.Vers[0].x+offx,r.Vers[0].y+offy),100,PaintableData::Blue,30);
                }
            }
...
public:
    ...
    void draw_obj(ObjectRect obj ,int offx,int offy,A draw_line ,B draw_text)
    {
        VdPoint p1(obj.x+offx,           obj.y+offy);
        VdPoint p2(obj.x+obj.w+offx,     obj.y+offy);
        VdPoint p3(obj.x+obj.w+offx,     obj.y+obj.h+offy);
        VdPoint p4(obj.x+offx,                obj.y+obj.h+offy);
        ...
        sprintf(buf,"%s(%d)",obj.label.data(),obj.confidence_rate);
        draw_text(buf,p1,1,PaintableData::Colour::Red,3);
              ...
              B draw_circle,C draw_text)
    {
        ...
        //ObjectRect  &r=MvdDetectedObjects[i];
        //   draw_line(p1,p2,color,2);
        //        }
        ...
        for(ObjectRect r:MvdDetectedObjects){
            ...
            draw_obj(r,offx,offy,draw_line,draw_text);

            //            rcts.push_back(VdRect(r.x+offx,r.y+offy,r.w,r.h));
            //              draw_line(p1,p2,color,2);
            //            draw_text()
            //            draw_text(,ps[0],1,PaintableData::Colour::Red,3);
