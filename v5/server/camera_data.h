...
public:
    ...
    virtual bool right_press(VdPoint pnt,vector<right_press_menu_item>&items_ret)
    {
        ...
        for(int i=0;i<DetectRegion.size();i++){
            ...
            if(dt.right_press(pnt,items_ret)){
                ...
                itm.pkt=get_request_pkt(CameraInputData::DELETE_REGION,i+1,\
                                        JsonPacket());
                itm.text="del this region";
        ...
        itm1.pkt=get_request_pkt(CameraInputData::INSERT_REGION,DetectRegion.size(),\
                                 DetectRegionInputData::get_region_test_data().data());
        itm1.text= "append region";
            ...
            B draw_circle,C draw_text)
    {
        ...
        if(ClientConfig::show_camera_state){
            draw_text(Url,VdPoint(200,200),100,PaintableData::Yellow,30);
        }
    }
